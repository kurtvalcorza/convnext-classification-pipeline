"""Static release-asset validation for the ConvNeXt-Tiny in12k_ft_in1k classification DIMER pipeline.
...
CODE_MARKERS = (
    'from convnext_classification_pipeline import MAX_BATCH, MAX_IMAGE_SIDE, NUM_CLASSES',
    'from convnext_classification_pipeline import MODEL_ID, MODEL_KEY, MODEL_REVISION, '
    'ConvNeXtClassificationPipeline, stage_missing_files, top_k_accuracy, verify_snapshot',
    "WEIGHTS_DIR = ROOT / 'weights' / MODEL_KEY",
    'fetched = stage_missing_files(WEIGHTS_DIR, allow_download=True)',
    'snapshot = verify_snapshot(WEIGHTS_DIR)',
    'pipe = ConvNeXtClassificationPipeline.from_pretrained(weights_dir=WEIGHTS_DIR)',
    'result = pipe.predict(image, top_k=5)',
    "top_k_accuracy(result['predictions'], [ground_truth], k=1)",
    "top_k_accuracy(result['predictions'], [ground_truth], k=5)",
    "print({'ceilings': {'NUM_CLASSES': NUM_CLASSES, 'MAX_IMAGE_SIDE': MAX_IMAGE_SIDE, "
    "'MAX_BATCH': MAX_BATCH}})",
...
MARKDOWN_MARKERS = (
    ...
    'is computed only when a ground-truth class index was supplied',
    'On the synthetic default sample no metric exists and none is reported',
